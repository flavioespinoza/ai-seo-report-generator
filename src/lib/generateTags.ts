...
export function detectBusinessCategory(metadata: Metadata, aiFeedback: string, url: string): string {
	const content = `${metadata.pageTitle} ${metadata.metaDescription} ${metadata.h1Tags.join(' ')} ${aiFeedback}`.toLowerCase()
